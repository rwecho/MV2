...
final searchResultsProvider = FutureProvider.autoDispose<List<V2Topic>>((ref) {
  final query = ref.watch(searchQueryProvider).trim();
  if (query.isEmpty) return Future<List<V2Topic>>.value(const <V2Topic>[]);
  final sort = ref.watch(searchSortProvider);
  return ref.watch(v2exApiProvider).searchTopics(query, sort: sort.sov2exValue);
}, retry: mv2Retry);
