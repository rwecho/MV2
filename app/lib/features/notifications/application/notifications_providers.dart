...
///
/// 屏蔽用户 is applied here: a blocked actor's rows never reach the UI (and an
/// emptied day section disappears with them). The tab-bar badge still reflects
/// the server's count, which is expected for a purely local block list.
final notificationsPageProvider = FutureProvider.family<NotificationPage, int>((
  ...
  if (blocked.isEmpty || result.isSignedOut) return result;
  return result.copyWith(
    groups: BlockedContent.notificationGroups(blocked, result.groups),
  );
}, retry: mv2Retry);
...
final notificationsProvider = FutureProvider<NotificationPage>((ref) async {
  final result = await ref.watch(v2exApiProvider).notifications();
  final blocked = ref.watch(blockedUsersProvider);
  if (blocked.isEmpty || result.isSignedOut) return result;
  return result.copyWith(
    groups: BlockedContent.notificationGroups(blocked, result.groups),
